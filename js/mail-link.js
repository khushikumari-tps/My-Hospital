...
(function () {
    ...
    var CSS = '' +
        ...
        '.aml-box strong,.aml-addr{color:#C94239;font-weight:700;word-break:break-all}' +
        '.aml-addr{text-decoration:none;border-bottom:1px solid rgba(201,66,57,.4)}' +
        '.aml-addr:hover{border-bottom-color:#C94239}' +
    ...
    function panel(m) {
        ...
        /* One way out, not a menu of them. The address itself is the same
           link as the button, so tapping what the reader is already looking
           at opens the compose window — the subject and body ride along. */
        veil.innerHTML =
            '<div class="aml-box">' +
            '  <h3>No mail app opened</h3>' +
            '  <p>Your browser has no email program set up. Write to us at<br>' +
            '    <a class="aml-addr" href="' + esc(gmail) + '" target="_blank" rel="noopener">' +
            esc(m.to) + '</a></p>' +
            '  <div class="aml-acts">' +
        ...
        veil.querySelector('.aml-acts a').focus();
    }
