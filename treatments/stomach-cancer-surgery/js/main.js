/* Advitya Healthcares — Stomach Cancer Surgery page interactions */
(function () {
  'use strict';
  ...
  /* ---------- appointment form (Full Name / Phone / Email / Date / Message) ---------- */
  var apptForm = $('#appt-form');
  if (apptForm) {
    var apptStatus = $('#appt-status');
    var apptDate = $('#appt-date');
    if (apptDate) apptDate.min = isoOf(new Date());
    ...
    $$('input, textarea', apptForm).forEach(function (f) {
      f.addEventListener('input', function () { showError(f, ''); });
    });

    apptForm.addEventListener('submit', function (e) {
      e.preventDefault();
      var name  = $('#appt-name');
      var phone = $('#appt-phone');
      var email = $('#appt-email');
      var date  = apptDate;
      var firstBad = null;
      var fail = function (field, msg) { showError(field, msg); if (!firstBad) firstBad = field; };

      if (!name.value.trim() || name.value.trim().length < 2) fail(name, 'Please enter your full name.');
      if (!/^[6-9][0-9]{9}$/.test(normalisePhone(phone.value))) fail(phone, 'Please enter a valid 10-digit Indian mobile number.');
      if (!email.value.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email.value.trim())) fail(email, 'Please enter a valid email address.');
      if (!date.value) fail(date, 'Please choose a preferred date.');
      else if (date.value < isoOf(new Date())) fail(date, 'Please choose today or a future date.');

      if (firstBad) { firstBad.focus(); return; }

      try {
        sessionStorage.setItem('adv-appt', JSON.stringify({
          name: name.value.trim(), phone: normalisePhone(phone.value), email: email.value.trim(),
          date: date.value, location: ($('input[name="appt-location"]:checked') || {}).value || ''
        }));
      } catch (err) {}

      if (apptStatus) { apptStatus.textContent = 'Submitting your request …'; apptStatus.classList.add('is-visible'); }
      var submit = $('button[type="submit"]', apptForm);
